...
\begin{frame}{迈克尔·拉宾}
	...
	\only<2>{\xiaowuhao{
		\begin{itemize}
			...
			\item \textcolor{red}{1959年 与达纳斯·斯科特共同发表“有限自动机与其判定性问题”(28岁)}
...
\only<2>{\xiaowuhao{\begin{itemize}
		...
		\item \textcolor{red}{1959年与迈克尔·拉宾共同发表“有限自动机与其判定性问题”(27岁)}
...
\begin{frame}{DFA的局限性}
	确定有限自动机每一个状态后面的状态都是确定的，但是现实生活中有很多时候选择并不唯一。\pause
	
	当我们在餐馆，菜单上有许多菜品，我们可能选择甜点，也可能选择主菜，这不是一个随机过程，但是确实是非确定的。\pause
	当我们选择了某一个种菜品后，我们就进入了另一个状态，结果就是满意或者不满意，也就是接受或者拒绝。\pause
	
	因此针对输入只要有任何一种可能达到的“可接受状态”，“非确定性”的机器就会“接受”并运行下去。
\end{frame}
	...
	子集构造法就是依次对NFA的状态求闭包运算，闭包中的元素作为一个新状态，然后计算新状态之间的关系。\pause
		...
		\begin{figure}[htbp]
			\begin{minipage}{0.4\textwidth}
				\begin{tikzpicture}[shorten >= 1pt, node distance=1cm, on grid, >=stealth]
					...
					\path[->] (s_3) edge[bend right=45] node[above]{$\epsilon$} (s_1)
							(s_1) edge[bend right=45] node[above]{$\epsilon$} (s_3);
					\path[->] (s_4) edge node[above]{b} (s_5)
							(s_5) edge node[above]{a} (s_6);
			...
			\uncover<4->{
				...
				\end{tikzpicture}
					\caption*{\qihao{其中$T_0 = \{0,1,3,4,6,7\}$\\$T_1=\{2\}$,$T_2=\{5\}$\\$T_3=\{1,3,7\}$,$T_4=\{4,6,7\}$}}
			}
			\end{minipage}
		\end{figure}
		}
...
%	\begin{example}
%		\begin{itemize}
%		\item<2-> $T_0 = \epsilon-closure(0) = \{0,1,3,4,6,7\}$, $move(T_0, a) = \{ 2\}$,$move(T_0, b) = \{5\}$
%		\item<3-> $T_1 = \epsilon-closure(2) = \{ 2\}$,$move(T_1, b) = \{3\}$
%		\item<4-> $T_2 = \epsilon-closure(5) = \{5\}$, $move(T_2, a) = \{6\}$
%		\item<5-> $T_3 = \epsilon-closure(3) = \{1,3,7\}$,$move(T_3,a) = \{2\}$
%		\item<6-> $T_4 = \epsilon-closure(6) = \{ 4,6,7\}$, $move(T_4,b) = \{5\}$
%		\item<7-> $D(T_0, a) = T_1, D(T_0, b) = T_2, D(T_1, b) = T_3$\\
...
\subsection{“世界末日”}
\begin{frame}{固有难度}
	平方取数的你运算可做，但是难度非常大，拉宾就提出了计算固有难度这一说法。\pause
	
	有些运算是可计算的，但是由于其固有难度非常高，你几乎无法在有限时间内得出结论，所以它的可计算性也就是失去意义了。\pause
	
	%就好比不管你用动滑轮还是定滑轮提起物体，都至少要克服重力做过，这就是提物体固有难度
%	\begin{figure}
%		\includegraphics{figures/gynd.eps}
%		\caption*{\tiny 固有难度克服重力做功$mgh$}
%	\end{figure}
\end{frame}

\begin{frame}{“世界末日”}
	在拉宾那个年代，人工智能界有不少科学家还在执着于编写程序来验证“匹斯伯格系统的原理”。\pause
	
	\textcolor{red}{拉宾干了一件很坑爹的事情！}\pause
	
	证明了一个哪怕只有100符号的系统，即使用1万台每秒一亿次的计算机，也要运行一万一年。\pause
	
	当拉宾的演讲结束，研究人工智能的人们都在惊呼这是世界末日，因为他们的实验室可能再也不会得到资助了。
	\begin{figure}
