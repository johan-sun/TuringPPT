...
\begin{frame}{迈克尔·拉宾}
	...
	\only<2>{\xiaowuhao{
		\begin{itemize}
			...
			\item 1979年 拉宾发明第一个非对称密码系统—拉宾密码系统(48岁)
			\item 1981年 拉宾提出“不经意传输”技术(50岁)
...
\only<2>{\xiaowuhao{\begin{itemize}
		...
		\item 1997年获皇家瑞典皇家科学院颁发的罗尔夫·朔克逻辑和哲学将(65岁)
...
\begin{frame}{注定相遇}
	\begin{itemize}\wuhao{
		...
		\pause
		\item 1957年，拉宾还在完成自己的论文，IBM研究院为他提供了一份暑期工作，让他自由从事任何感兴趣的项目。
		\pause
		\item 斯考特是逻辑学方面的天才，他们两个人一起研究，提出了一个概念：能够“猜测”的计算机。\pause
		\item 他们的合作是非凡的，旺旺一个人先提出问题，然后各自回到自己的角落，然后另一个人很快就会给出解决方案。}
	\end{itemize}
\end{frame}
...
\end{frame}
\begin{frame}
\wuhao{拉宾采用了一种叫做『单项』函数的算法得出了觉得方案。所谓的单项函数，就是一个方向的计算很容易，但是反过来很难。}
...
\begin{frame}{固有难度}
	...
	证明了一个哪怕只有100符号的系统，即使用1万台每秒一亿次的计算机，也要运行一万一年\pause，他妈还不止！\pause
...
\begin{frame}{大数素数判断}
	对于一个很大的数n来说
	\begin{itemize}
	\item 从$[1, n-1 ]$中随机取150一个数$r$
	\item 一次验证其是否是新式见证者
	\item 如果数新式见证者说明$n$是合数
	\item 否则认为$n$是素数
	\end{itemize}
\end{frame}
...
\begin{frame}{错误的概率}
	...
	\uncover<3->{也就是说每一次检验都随机到了$1\over4$的非新式见证者。}
	\uncover<4->{那么150次检验都错误的概率是
	\begin{equation*}
